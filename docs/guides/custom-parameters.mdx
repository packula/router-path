...
*portion* of the path. This expression **must** have exactly one [capturing
group].
...
argument into a string that makes up a *portion* of a path.
...
[`Name`][name type] type variable for the parameter name instead of using a
simple `string`. This [literal type] contains the name of the parameter, but in
a way that TypeScript's type system understands. This is a key part of how
TypeScript can know about the *existence* of route parameters.
...
### `Arg` and `Result` types {#arg-and-result-types}

The [`Param`] interface is a [generic] type. Two of its type variables are
closely related:

- The [`Arg`] type variable is the type of argument accepted by [`format`].
- The [`Result`] type variable is the type of value returned by [`parse`].

It's common for these two types to be identical for simple parameters, but in
some cases, you may wish to have different definitions for these types.

For example, it's possible to make the integer-based parameters used as an
example in the [Matching and parsing] section take numbers *or* strings when
building paths, but continue to return numbers when matching paths. All we need
to do is specify an [`Arg`] as `number | string` and [`Result`] as `number`:
...
[`result`]: ../api/type/Param.mdx#type-variable-result
[matching and parsing]: #matching-and-parsing
